...
for task_link in task_links:
    #  make soup out of each link and throw some sweet ass text
    task_soup = make_soup(task_link)
    task_soup_intro = task_soup.find('div', class_='minh')
    print(task_soup_intro.p.text + '\n')
    examples = [sibling.string for sibling in
                task_soup_intro.next_siblings if sibling.string is not None]
